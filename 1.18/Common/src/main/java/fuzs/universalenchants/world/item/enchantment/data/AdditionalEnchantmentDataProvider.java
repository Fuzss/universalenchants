...
public class AdditionalEnchantmentDataProvider {
    ...
    public Map<Enchantment, List<DataEntry<?>>> getEnchantmentDataEntries() {
        if (this.defaultCategoryEntries == null) {
            ...
            Map<Enchantment, DataEntry.Builder> builders = Registry.ENCHANTMENT.stream().collect(Collectors.toMap(Function.identity(), CommonAbstractions.INSTANCE::defaultEnchantmentDataBuilder));
