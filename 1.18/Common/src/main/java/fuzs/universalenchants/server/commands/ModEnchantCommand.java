...
public class ModEnchantCommand {
	...
	private static int enchant(CommandSourceStack commandSourceStack, Collection<? extends Entity> collection, Enchantment enchantment) throws CommandSyntaxException {
		return enchant(commandSourceStack, collection, enchantment, enchantment.getMaxLevel());
	}
			...
			if (collection.size() == 1) {
				commandSourceStack.sendSuccess(
						level > 0 || !UniversalEnchants.CONFIG.get(CommonConfig.class).enchantCommand.fixEnchantCommand
								? new TranslatableComponent("commands.enchant.success.single", enchantment.getFullname(level), collection.iterator().next().getDisplayName())
								: new TranslatableComponent("commands.enchant.remove.success.single", getEnchantmentName(enchantment), collection.iterator().next().getDisplayName()), true
				);
			} else {
				commandSourceStack.sendSuccess(
						level > 0 || !UniversalEnchants.CONFIG.get(CommonConfig.class).enchantCommand.fixEnchantCommand
							? new TranslatableComponent("commands.enchant.success.multiple", enchantment.getFullname(level), collection.size())
							: new TranslatableComponent("commands.enchant.remove.success.multiple", getEnchantmentName(enchantment), collection.size()), true);
			}
